...
function editQuestionSet(id) {
    const set = questionSets.find(s => s.id === id);
        const editForm = `
            <div id="editSetForm" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
                <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl max-w-3xl w-full">
                    ...
                    <form id="editQuestionSetForm">
                        ...
                        <div class="mb-4">
                            ...
                            <button type="button" id="addQuestionBtn" class="mt-2 bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded">
                                Add Question
                            </button>
                        </div>
...
function getQuestionsFromForm() {
    const questions = [];
        ...
        questionItems.forEach((item) => {
            ...
            questions.push({ id: questionId, name: questionName, options: options });
        });
